...
A Manifest V3 Chrome extension that renders any raw `.md` file you land on across the web — a GitHub raw link, a gist, a doc served straight off S3 — with Amarantha's rich view, instead of Chrome's plain-text viewer. Opens read-only; an **Edit** toggle unlocks in-place editing, and **Save** downloads the edited markdown as a local file under its original name. Nothing ever writes back to the page it came from — there's no file-system access here, by design; that's the native desktop app's job. Settings (theme, appearance, text size, an on/off switch) live in `options.html`.

## How it decides when to render

`manifest.json`'s `content_scripts` matches any URL ending in `.md` over http/https (`*://*/*.md`) or a local file (`file:///*.md`) — `*://` only ever expands to `http`/`https` in Chrome's match-pattern syntax, never `file:`, so that second pattern is required for `file:///path/to/doc.md` links to match at all. That still includes pages like GitHub's `/blob/main/README.md` UI, which is real HTML, not a raw file. The content script (`src/content/detect.ts`) only takes over when the page is actually raw text — `document.contentType` is `text/plain`/`text/markdown`, or (content type unset) the whole `<body>` is Chrome's own single generated `<pre>` wrapper. Anything else is left untouched.
